...
car_ctrl_cmd_t *CheckUserCmd(uint8_t *buf) {
    ...
    for (i = 0; i < sizeof(g_CarCtrlCmd) / sizeof(car_ctrl_cmd_t); i++) {
        if (strncmp((const uint8_t *) g_CarCtrlCmd[i].cmd, (const char *) buf,
                    strlen((const char *) g_CarCtrlCmd[i].cmd)) == 0) {
...
/*
 *    UART Rx Callback for command parse
 *    @brief use @ and & to tag command start and end use | to divide different command
 *
 */
